...
class ReadMincreaftLog: #Might break in anthor version (cần tùy chỉnh theo log của từng phiên bản)
	file = "C:\\Users\\Admin\\AppData\\Roaming\\.minecraft\\logs\\latest.log"
	#file = "C:\\Users\\Admin\\Desktop\\2021-07-13-3.log"
	
	AllMode = ["3fmc Before Login", "3fmc Login Lobby", "3fmc Bedwars Lobby", "3fmc Bedwars Waiting", "3fmc Bedwars Mode", "3fmc Bedwars (Died)", "3fmc Bedwars Trading without resources", "3fmc Bedwars Mode (Bed Destroyed)", "3fmc Bedwars Ended", "Minecraft Skyblock Mode"]
	...
	def __init__(self):
		...
		self.currentTime = dt.time(today.hour, today.minute, today.second) #Init time
		...
		self.DEAD = 0
		self.KILL = 0
		self.LastChatSpam = [None, dt.time(today.hour, today.minute, today.second)][0]
	...
	def progess(self, line):
		...
		print(self.mode, self.Up_KIEM, self.Up_GIAP, self.ACTIVE_IT_TRAP, self.IT_TRAP, line)
		...
		elif "Không ?? tài nguyên! B?n c?n thêm " in line:
			self.mode = self.AllMode[7]
