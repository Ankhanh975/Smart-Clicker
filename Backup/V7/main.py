...
def WhatToChat(LogReader, KeyPressed: int, mode = ["Bedwars", "Skyblock"][0]):
	...
	if KeyPressed==1:
		...
		elif LogReader.Up_KIEM == True and LogReader.Up_GIAP==0:
			if PlayedTime < 0.6:
				return "@."
			elif 0.6 < PlayedTime < 4.5:
				return random.choice(["up GIAP nua pls", "up GIAP pls"])
			elif PlayedTime > 4.5:
				return random.choice(["up GIAP nua di", "up GIAP pls, no trau vcl"])
			elif PlayedTime > 6:
				return random.choice(["up GIAP pls", "up GIAP nhanh pls"])
		elif LogReader.Up_KIEM == True and LogReader.Up_GIAP==0:
'''
def WhatToChat(LogReader, KeyPressed: int, mode = ["Bedwars", "Skyblock"][0]):
	...
	if KeyPressed==2:
		if LogReader.Up_KIEM == False:
			...
			if LogReader.Up_GIAP == 3:
				...
				return ["up GIAP III nua pls"]
			else:
				return ["up GIAP pls"]
	elif KeyPressed==3:
		if LogReader.ACTIVE_IT_TRAP == None:
			return ["ai bed?", "bed di nhe"]
		else:
			return ["bed lai di", "bed 1 lop nua vcl", "bed 1 lop nua di"]
	elif KeyPressed==1:
		return ["@ghe qua ", "@ghe vay ", "@ghe vay?", "@cc", "@cc?", "@?"]
	elif KeyPressed==4:
		return ["E, ve di bed kiaa", "E bedd", "Bed kia ve di", "ve"]
	elif KeyPressed==5:
		return ["@gg", "@hay", "@ay"]
	elif KeyPressed==6:
		return ["/leave"]
...
class Screen(RunThread):
	...
	def SelfResize(self): #TODO: Performance
		...
		win32gui.MoveWindow(self.hwnd, x0, y0, self.PixelwinSize[0], self.PixelwinSize[1], True)
